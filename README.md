...
- **Gestion des Cours (UI)** ✅ — [CoursGestionPane](src/main/java/com/teacherflow/ui/CoursGestionPane.java) : créer/renommer/colorer un cours, lui attacher des fichiers (sélection individuelle, import d'un dossier entier ou lien web) ou en retirer plusieurs à la fois, supprimer un cours (icône corbeille). Recherche/filtre par nom ou tag ; étiquettes libres par fichier. Branché dans [App.java](src/main/java/com/teacherflow/app/App.java) qui charge/sauvegarde automatiquement les données à chaque modification et à la fermeture. Testé manuellement dans l'application.
...
- [ ] Dossier réel du disque lié à un Cours (suivi, pas juste un import ponctuel), avec actualisation pour importer les nouveaux fichiers sans dupliquer l'existant
