...
`App.appliquerTheme()`. Le logo est aussi affiché à côté du titre dans la barre latérale.

## État actuel du projet

- **Socle (modèle + persistance)** ✅ — [Fichier](core/src/main/java/com/courseflow/model/Fichier.java), [Cours](core/src/main/java/com/courseflow/model/Cours.java), [Creneau](core/src/main/java/com/courseflow/model/Creneau.java), [EmploiDuTemps](core/src/main/java/com/courseflow/model/EmploiDuTemps.java) ; persistance JSON via [DataStore](core/src/main/java/com/courseflow/persistence/DataStore.java) (Jackson) dans `~/.courseflow/data.json` ; couvert par des tests unitaires (`mvn test`). Écriture atomique (fichier `.tmp` + renommage) et copies de secours rotatives (`data.json.bak1`..`bak3`) pour qu'une interruption ne tronque jamais l'unique fichier ; un `data.json` présent mais illisible est mis de côté (`.corrompu-<horodatage>`) plutôt qu'ignoré silencieusement — l'application le signale et démarre à vide, la CLI s'arrête en erreur. Champs JSON inconnus tolérés (compat ascendante).
...
- [x] Lien de fichiers entre cours : un cours peut lier des fichiers depuis n'importe quel autre cours existant (sélecteur de cours source dans le panneau "Lier des fichiers depuis un autre cours"), pas seulement un unique "cours par défaut" comme au départ — le cours par défaut désigné dans les Paramètres reste juste la présélection pratique de ce sélecteur. Pas de risque de cycle malgré le croisement possible entre cours : un fichier lié (`fichiersLies`, une liste d'UUID sur `Cours`) est toujours résolu en un seul saut contre les fichiers réellement *possédés* par un cours ([EmploiDuTemps.trouverFichierPartage](core/src/main/java/com/courseflow/model/EmploiDuTemps.java)), jamais en suivant récursivement les liens d'un autre cours — ni les dossiers virtuels (abandonnés au profit des tags)
